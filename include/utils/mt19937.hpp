...
// ---------------------------------------------------------------------------
// MT19937
//   Wrapper class for the STL mt19937_64 random number generator. Stores the
//   current seed of the engine, and also has class method to produce a
//   random double over the interval [0,1).
// ---------------------------------------------------------------------------
class MT19937 : public RNG {
 public:
  MT19937() : engine(), unit_dist(){};
  MT19937(uint64_t seed_) : engine(), unit_dist() { engine.seed(seed_); }
  ...
  // -----------------------------------------------------------------------
  void seed(uint64_t seed_) { engine.seed(seed_); }
