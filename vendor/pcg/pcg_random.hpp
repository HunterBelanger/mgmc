...
public:
    ...
    // This function gets the current seed of the engine
    itype get_seed()
    {
        return this->state_;
    }
    
    // This function directly sets the state varible (seed)
    // without performing a bump like the seed() method does.
    // This function with get_seed() allows user to make a new
    // engine starting where another stopped.
    void set_seed(itype _state_)
    {
      this->state_ = _state_;
    }
