...
std::vector<Particle> DeltaTracker::transport(
    ...
    for (size_t n = 0; n < bank.size(); n++) {
      ...
      while (p.is_alive()) {
        ...
        while (virtual_collision) {
          double d_coll = rng->exponential(Emaj[p.E()]);
          ...
          if (d_coll < surf_bound.distance) {
            ...
            trkr.move(d_coll);
            trkr.get_current();
            if (rng->rand() < trkr.material()->Et(p.r(), p.E()) / Emaj[p.E()])
...
void DeltaTracker::russian_roulette(Particle& p, std::shared_ptr<RNG> rng) {
...
void DeltaTracker::scatter_particle(Particle& p, std::shared_ptr<Material> mat,
...
Particle DeltaTracker::fission_neutron(Particle& p,
